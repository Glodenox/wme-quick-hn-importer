...
const SHORTCUT_ID = 'quickhnimportercomplete';
    ...
    response.response.features?.forEach((feature) => {
      features.push({
        ...
        properties: {
          ...
          number: feature.properties.huisnummer + feature.properties.huisletter,
...
function updateLayer() {
  ...
  Repository.getExtentData(wmeSDK.Map.getMapExtent()).then((features) => {
    ...
    if (selectedStreetNames.length > 0) {
      autocompleteFeatures = features.filter(feature => selectedStreetNames.includes(feature.properties.street.toLowerCase()) &&
                                             (!streetNumbers.has(feature.properties.street.toLowerCase()) || (streetNumbers.has(feature.properties.street.toLowerCase()) && !streetNumbers.get(feature.properties.street.toLowerCase()).has(simplifyNumber(feature.properties.number)))) &&
